...
class Stats(XrayAPIBase):
    ...
    async def __query_stats(self, pattern: str, reset: bool = False) -> typing.List[StatResponse]:
        ...
        try:
            stub = command_grpc.StatsServiceStub(self._channel)
            response = await stub.QueryStats(command_pb2.QueryStatsRequest(pattern=pattern,
                                                                           reset=reset))
    ...
    async def get_user_stats(self, email: str, reset: bool = False) -> UserStatsResponse:
    ...
    async def get_inbound_stats(self, tag: str, reset: bool = False) -> InboundStatsResponse:
    ...
    async def get_outbound_stats(self, tag: str, reset: bool = False) -> OutboundStatsResponse:
